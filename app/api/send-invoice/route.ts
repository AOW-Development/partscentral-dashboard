...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log("productInfo[0] prices:", {
      partPrice: invoiceData.productInfo?.[0]?.partPrice,
      taxesPrice: invoiceData.productInfo?.[0]?.taxesPrice,
      handlingPrice: invoiceData.productInfo?.[0]?.handlingPrice,
      processingPrice: invoiceData.productInfo?.[0]?.processingPrice,
      corePrice: invoiceData.productInfo?.[0]?.corePrice,
    });
...
async function generateInvoicePDF(data: InvoiceData) {
  ...
  const locationBytes1 = fs.readFileSync(
    path.join(process.cwd(), "public", "location.png")
  );
  const websiteBytes1 = fs.readFileSync(
    path.join(process.cwd(), "public", "website.png")
  );
  const phoneBytes1 = fs.readFileSync(
  ...
  if (data.productInfo && data.productInfo.length > 0) {
    for (const product of data.productInfo) {
      ...
      const lines = [];
      const mainDescription = `${product.year || ""} ${product.make || ""} ${
        product.model || ""
      } ${product.parts || ""}`;
      ...
      if (product.specification) {
        ...
        const wrappedSpecLines = getWrappedText(
      ...
      if (data.customerInfo.vinNumber) {
        const vinText = `VIN Number: ${
          data.customerInfo.vinNumber || "17 Alpha Digits"
        }`;
        const wrappedVinLines = getWrappedText(
          vinText,
          itemDescriptionWidth,
          times,
          10
      ...
      // --- Dynamic Height with Padding ---
      const productDescriptionHeight =
        numLines * lineHeight + verticalPadding * 2;
      ...
      const formatItemPrice = (price: string | number | undefined): string => {
        if (!price || price === "0" || price === 0) return "0.00";
        const num = typeof price === "string" ? parseFloat(price) : price;
        return isNaN(num) ? "0.00" : num.toFixed(2);
      };

      const formattedPrice = formatItemPrice(data.productInfo[0].partPrice);
      const formattedTotal = formatItemPrice(
        data.customerInfo.totalSellingPrice
      );
  ...
  const textWidth = (text: string, size: number, font: any): number =>
    font.widthOfTextAtSize(text, size);

  const numericTotal = +data.customerInfo.totalSellingPrice;
  const formattedTotal = !isNaN(numericTotal)
  ...
  // Core
  const coreText = `Core: $${formatPrice(data.productInfo[0].corePrice)}`;
  ...
  // Tax
  const taxText = `Tax: $${formatPrice(data.productInfo[0].taxesPrice)}`;
  page.drawText(`|`, {
    x: currentX,
    y: feesY,
    size: 10,
  ...
  // Handling Fee
  const handlingText = `Handling Fee: $${formatPrice(
    data.productInfo[0].handlingPrice
  )}`;
  page.drawText(`|`, {
    x: currentX,
    y: feesY,
  ...
  // Processing Fee
  const processingText = `Processing Fee: $${formatPrice(
    data.productInfo[0].processingPrice
  )}`;
  page.drawText(`|`, {
    x: currentX,
    y: feesY,
  ...
  if (data.paymentInfo.cardNumber) {
    page.drawText(
      `Payment Mode: Card: **** **** **** ${data.paymentInfo.cardNumber.slice(
        -4
      )}`,
      {
        x: 40,
